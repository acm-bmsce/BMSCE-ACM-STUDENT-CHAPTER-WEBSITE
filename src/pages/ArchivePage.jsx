...
import { 
  Search, Calendar, MapPin, ArrowLeft, Loader2, X, ExternalLink, 
  Clock, Users, ChevronDown, ChevronUp, Camera, Maximize, 
...
export default function ArchivePage() {
    ...
    const fetchProgressively = async () => {
      try {
        ...
        while (isMounted) {
          ...
          if (data.length === 0) { setIsSyncing(false); break; }
          const today = new Date();
          const pastBatch = data.map(ev => ({
            ...ev,
          ...
          skip += limit;
          if (data.length < limit) { setIsSyncing(false); break; }
        }
      } catch (e) { if (isMounted) setFetchStatus("error"); }
    };
    fetchProgressively();
 ...
 useEffect(() => {
  ...
  const filteredEvents = useMemo(() => {
    return allPastEvents.filter(ev => ev.title.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [searchQuery, allPastEvents]);

  const galleryPhotos = selectedEvent?.gallery || selectedEvent?.images || selectedEvent?.photos || [];
    ...
    <main className="min-h-screen bg-[#030303] pt-32 pb-20 px-6 font-general text-white">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <header className="mb-16">
          <button onClick={() => window.history.back()} className="flex items-center gap-2 text-gray-500 hover:text-[#7DD4EF] transition-colors mb-8 uppercase text-[10px] font-bold tracking-widest">
          ...
          <div className="mt-4 flex items-center gap-4 text-gray-500 text-xs uppercase tracking-widest font-bold">
            {fetchStatus === "success" ? `History Archive / ${allPastEvents.length} Nodes` : 'Initializing Systems...'}
            {isSyncing && <Loader2 size={12} className="animate-spin text-[#7DD4EF]" />}
          </div>
        </header>

        <div className="mb-12 relative w-full md:w-[450px]">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" size={18} />
          <input type="text" placeholder="Search parameters..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-[#7DD4EF]/40 transition-all placeholder:text-gray-700" />
        </div>

        {fetchStatus === "loading" ? (
          <div className="py-40 flex flex-col items-center gap-4"><Loader2 className="animate-spin text-[#7DD4EF]" size={32} /><p className="text-[#7DD4EF] text-[10px] font-bold tracking-widest uppercase">Fetching Data...</p></div>
        ) : filteredEvents.length === 0 ? (
          <div className="py-32 text-center border border-dashed border-white/10 rounded-3xl text-gray-600 uppercase text-xs font-bold tracking-widest">No entries match query</div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.map((ev) => (
              <div key={ev._id || ev.id} onClick={() => setSelectedEvent(ev)} className="group bg-[#0A0A0A] border border-white/10 rounded-3xl overflow-hidden hover:border-[#7DD4EF]/40 transition-all duration-500 cursor-pointer">
                <div className="h-56 overflow-hidden relative">
                  <img src={ev.imageUrl || ev.image || "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4"} loading="lazy" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" alt={ev.title} />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A] to-transparent opacity-60" />
                </div>
                <div className="p-8">
                  <div className="flex justify-between mb-4">
                    <span className="text-[#7DD4EF] text-[9px] font-black uppercase tracking-widest border border-[#7DD4EF]/20 bg-[#7DD4EF]/5 px-3 py-1 rounded-md">{ev.tag}</span>
                    <span className="text-gray-600 text-[10px] font-bold">{ev.parsedDate.getFullYear()}</span>
                  </div>
                  <h3 className="text-3xl font-bebas-neue text-white mb-3 group-hover:text-[#7DD4EF] transition-colors">{ev.title}</h3>
                  <p className="text-gray-500 text-sm leading-relaxed line-clamp-2">{ev.description}</p>
                </div>
              </div>
            ))}
          </motion.div>
              ...
              >
                ...
                >
                  <div className="p-8 md:p-12">
                    <div className="mb-8">
                      <span className="text-[#7DD4EF] text-[10px] font-bold uppercase tracking-[0.3em]">{selectedEvent.tag}</span>
                      <h2 className="text-5xl md:text-7xl font-bebas-neue uppercase text-white mt-2 leading-[0.95]">{selectedEvent.title}</h2>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-8 mb-10 pb-8 border-b border-white/5 text-gray-400">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{selectedEvent.location || "BMSCE Campus"}</span>
                      </div>
                    </div>
                    ...
                    {galleryPhotos.length > 0 && (
                      <div className="mb-10">
                        <h4 className="text-white font-bebas-neue text-2xl mb-4 tracking-widest uppercase flex items-center gap-2">
                          <Camera size={20} className="text-[#7DD4EF]"/> Gallery
                        </h4>
                        <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar snap-x">
                          {galleryPhotos.map((p, i) => (
                            <div key={i} onClick={() => setFullscreenIdx(i)} className="relative w-48 h-36 shrink-0 rounded-2xl overflow-hidden border border-white/10 cursor-pointer snap-start group bg-black">
                              <img src={p} className="w-full h-full object-cover transition-transform group-hover:scale-110" alt="Gallery item" />
                    ...
                    {selectedEvent.speakers && (
                      <div className="mb-10">
                        <h4 className="text-white font-bebas-neue text-2xl mb-4 tracking-widest uppercase flex items-center gap-2">
                          <Users size={20} className="text-[#7DD4EF]"/> Speakers
                        </h4>
