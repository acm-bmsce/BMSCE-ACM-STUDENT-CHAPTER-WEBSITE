...
export default function EventTitleSection({ featured }) {
  ...
  return (
    <section className="relative w-full h-auto flex flex-col justify-center items-center bg-[#030303] overflow-hidden pt-40 pb-16 font-general">
      ...
      <div className="max-w-7xl w-full mx-auto px-6 lg:px-10 relative z-10 flex-1 flex items-center">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
          ...
          <div className="hidden lg:flex justify-end items-center w-full">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut", delay: 0.2 }} className="relative w-full max-w-[500px] aspect-square will-change-transform group">
              
              
              <div className="absolute top-0 right-0 w-[80%] h-[80%] rounded-[2.5rem] border border-white/10 overflow-hidden bg-gray-900 shadow-2xl z-10 isolate"
              ...
              <motion.div 
                ...
                className="absolute bottom-[5%] left-0 w-[60%] h-[60%] rounded-[2.5rem] border border-white/10 overflow-hidden bg-[#0A0A0A] shadow-[0_20px_50px_rgba(0,0,0,0.8)] z-20 isolate"
                style={{ WebkitMaskImage: '-webkit-radial-gradient(white, black)', willChange: 'filter' }}
              >
                <img 
                  src={secondaryImg} 
              ...
              <motion.div 
                ...
                className="absolute bottom-[-2%] right-[2%] bg-black/80 backdrop-blur-xl border border-white/10 p-6 rounded-3xl flex flex-col gap-1 shadow-2xl z-30 min-w-[210px]"
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-2 h-2 rounded-full bg-[#7DD4EF] animate-pulse" />
                  <span className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Next Up</span>
                </div>
                
                <h3 className="text-white font-bebas-neue text-3xl md:text-4xl tracking-wide leading-[0.95] mb-1 line-clamp-2">
                  {featured?.title || "Upcoming Session"}
                </h3>
                <p className="text-[#7DD4EF] text-xs font-bold uppercase tracking-widest">{displayDate}</p>
              </motion.div>
