    ...
    <section className="bg-[#030303] py-32 relative overflow-hidden font-general text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-10">
        ...
        {fetchStatus === "loading" || fetchStatus === "waking" ? (
          ...
          >
            {sessions.map((session) => (
              ...
              >
                {/* 🚀 Main Cards: Starts color, hovers to black/white, mask added */}
                <div 
                  className="relative h-56 w-full overflow-hidden bg-gray-900 shrink-0 isolate"
        ...
        <div className="mt-20 flex justify-center">
          {/* 🚀 Button: Reduced size on mobile using sm: breakpoints */}
         <button
  onClick={() => (window.location.href = "/archive")}
  className="group relative flex items-center gap-2 sm:gap-4 px-5 py-3 sm:px-10 sm:py-5 bg-white text-black rounded-full font-normal font-bebas-neue text-lg sm:text-2xl uppercase tracking-widest hover:bg-[#7DD4EF] transition-all duration-300 overflow-hidden shadow-[0_0_40px_rgba(255,255,255,0.1)] hover:shadow-[0_0_40px_rgba(125,212,239,0.4)]"
>
  <span className="relative z-10 flex items-center gap-1.5 sm:gap-2 mt-0.5 sm:mt-1">
    Explore Full Archive <ArrowUpRight className="w-4 h-4 sm:w-6 sm:h-6 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
  </span>
  <div className="absolute inset-0 h-full w-0 bg-[#7DD4EF] transition-all duration-300 ease-out group-hover:w-full z-0" />
      ...
      {typeof document !== "undefined" && createPortal(
        <AnimatePresence>
          {selectedEvent && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 md:p-12 font-general text-white overflow-hidden">
              ...
              <motion.div
                ...
                className="relative w-full max-w-5xl bg-[#0A0A0A] border border-white/10 rounded-[24px] md:rounded-[32px] overflow-hidden shadow-2xl z-[10000] flex flex-col md:flex-row h-full max-h-[85vh]"
              >
                
                <button 
                  onClick={closeModal} 
                  className="absolute top-4 right-4 z-[10001] p-2.5 bg-black/50 hover:bg-[#7DD4EF] hover:text-black text-white rounded-full transition-all border border-white/10 backdrop-blur-md"
                >
                  <X size={18} />
                </button>

                
                <div className="w-full md:w-[35%] h-48 md:h-full relative shrink-0 bg-[#070707] border-b md:border-b-0 md:border-r border-white/5">
                  <img 
                ...
                <div 
                  className="w-full md:w-[65%] flex flex-col overflow-y-auto custom-scrollbar flex-1 relative"
                        ...
                        <div className="flex overflow-x-auto gap-4 pb-4 snap-x custom-scrollbar">
                          {galleryPhotos.map((photo, idx) => (
                            <div 
                              ...
                              className="relative w-48 md:w-56 aspect-[4/3] shrink-0 snap-start rounded-xl overflow-hidden border border-white/10 bg-black group cursor-pointer isolate"
                              style={{ WebkitMaskImage: '-webkit-radial-gradient(white, black)' }}
                            >
                              <img 
                                src={photo} 
          ...
          {fullscreenPhotoIndex !== null && (
            <div className="fixed inset-0 z-[20000] flex items-center justify-center p-4 md:p-8 font-general text-white cursor-auto">
              ...
              {galleryPhotos.length > 1 && (
                <>
                  <button 
                    onClick={(e) => { e.stopPropagation(); showPrevPhoto(e); }} 
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); showNextPhoto(e); }} 
